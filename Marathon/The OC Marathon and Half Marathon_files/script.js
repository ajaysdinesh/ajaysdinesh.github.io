...
function ActionFormCtrl($scope, $http, $timeout, $sce, $cookieStore, dataSources, dnnsf) {
    ...
    $scope.fnFactoryFilterByField = function (thisFieldName, filterByFieldName) {
        ...
        return function (o) {
            ...
            if ((thisField.selected && '/' + thisField.selected.filter != parentItem.path && thisField.selected.filter != parentItem.value.toString().replace('/', '\\/')) || (parentItem.filter && $scope.form.fields[filterByField.linkedTo].selected.path != "/" + parentItem.filter && $scope.form.fields[filterByField.linkedTo].selected.value.toString().replace('/', '\\/') != parentItem.filter)) {
                thisField.value = thisField.ddValue = thisField.tbValue = '';
                thisField.selected = null;
                return false;
            }
...
function initForm(formRoot, options, localization) {
    ...
    $.validator.addMethod("required-cblist", function (value, element) {
